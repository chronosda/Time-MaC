...
class EnhancedMultiMamba(nn.Module):
    """MultiMamba implementation for multimodal Time-MaC fusion."""
    def __init__(
        self,
        d_model: int,
        n_layer: int,
    ...
    def forward(self, input_audio, input_vision, input_text):
        ...
        output_vars = self.output_vars or seq_len
        ...
        for layer_idx in range(self.n_layer):
            ...
            vision_context = vision_b.mean(dim=1, keepdim=True)
            text_context = text_b.mean(dim=1, keepdim=True)
            # Broadcast vision/text to audio seq length for audio enhancement
            vision_broadcast = vision_context.expand(-1, n_vars, -1)
            text_broadcast = text_context.expand(-1, n_vars, -1)
            ...
            vision_enhanced = self.resnet_vision(
                torch.cat([audio_pooled, text_context], dim=-1)
            )
            text_enhanced = self.resnet_text(
                torch.cat([audio_pooled, vision_context], dim=-1)
            )
        ...
        # Reshape to forecasting output format [B, n_vars, pred_len].
        ...
        if predictions.dim() == 2:
            # Expand to [B, output_vars, pred_len].
            predictions = predictions.unsqueeze(1).expand(-1, output_vars, -1)

        return predictions


class CoupledMambaFusion(nn.Module):
    """Main Coupled Mamba fusion module for Time-MaC."""
    def __init__(self, config, vision_dim, text_dim, d_model):
        ...
        self.multimamba = EnhancedMultiMamba(
            ...
            output_vars=getattr(config, 'c_out', getattr(config, 'enc_in', None)),
